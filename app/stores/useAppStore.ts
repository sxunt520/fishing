...
interface AppState {//定义了应用的全局状态接口 `AppState`，包含用户信息、草稿内容、当前位置信息以及相关的更新方法。
  ...
  selectedMapSpot: any | null;
  setSelectedMapSpot: (spot: any | null) => void;
  convertedCandidateSpot: any | null;
  setConvertedCandidateSpot: (spot: any | null) => void;
}
  ...
  persist(
    (set) => ({//定义了应用的全局状态和相关的更新方法，包括用户信息、草稿内容和当前位置信息等。
      ...
      setCurrentLocation: (currentLocation) => {
        if (!isValidLocation(currentLocation)) return;
        set({ currentLocation });
      },//定义了一个 `setCurrentLocation` 方法，用于更新当前位置信息的状态。
      selectedMapSpot: null,
      setSelectedMapSpot: (selectedMapSpot) => set({ selectedMapSpot }),
      convertedCandidateSpot: null,
      setConvertedCandidateSpot: (convertedCandidateSpot) => set({ convertedCandidateSpot }),
    }),
